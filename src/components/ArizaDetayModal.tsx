...
export const ArizaDetayModal: React.FC<Props> = ({ ariza, sahaAdi, onClose }) => {
  ...
  const handleFotoTikla = (foto: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Tıklama olayının üst elementlere yayılmasını engelle
    setYukleniyor(true);
    const img = new Image();
    img.src = foto;
    img.onload = () => {
  ...
  const handleDuzenle = (e: React.MouseEvent) => {
    e.stopPropagation(); // Tıklama olayının üst elementlere yayılmasını engelle
    if (!canEdit) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50 overflow-y-auto" onClick={handleModalClick}>
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[95vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {canEdit && (
              <button
                ...
                className="inline-flex items-center px-2 sm:px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
              >
            ...
            {!ariza.cozum && canSolve && (
              <button
                ...
                className="inline-flex items-center px-2 sm:px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700"
              >
                        ...
                        {ariza.cozum.fotograflar.map((foto, index) => (
                          <div key={index} className="relative aspect-square">
                            <img
                              ...
                              onClick={(e) => handleFotoTikla(foto, e)}
                              onError={(e) => {
        ...
        {/* Çözüm Formu Modal */}
        {cozumFormAcik && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setCozumFormAcik(false)}>
