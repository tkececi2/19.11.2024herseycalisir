...
export const ArizaForm: React.FC<Props> = ({ ariza, onClose }) => {
  ...
  const [form, setForm] = useState({
    ...
    olusturmaTarihi: ariza ? format(ariza.olusturmaTarihi.toDate(), "yyyy-MM-dd'T'HH:mm") 
  ...
  useEffect(() => {
    const veriGetir = async () => {
      try {
        ...
        // Eğer düzenleme modundaysa mevcut fotoğrafları göster
        if (ariza?.fotograflar) {
          setOnizlemeler(ariza.fotograflar);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Eğer düzenleme modundaysa mevcut fotoğrafları da ekle
      if (ariza?.fotograflar) {
        fotografURLleri = [...ariza.fotograflar, ...fotografURLleri];
      }

      const arizaData = {
        ...form,
        durum: ariza?.durum || 'acik',
        olusturmaTarihi: ariza?.olusturmaTarihi || new Date(),
        guncellenmeTarihi: new Date(),
        olusturanKisi: ariza?.olusturanKisi || kullanici.id,
        fotograflar: fotografURLleri,
        yorumlar: ariza?.yorumlar || []
      };

      if (ariza) {
        // Güncelleme
        await updateDoc(doc(db, 'arizalar', ariza.id), arizaData);
  ...
  const handleFotografKaldir = (index: number) => {
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900">
            {ariza ? 'Arıza Düzenle' : 'Yeni Arıza Kaydı'}
          </h2>
          <button
            onClick={onClose}
              ...
              <div className="mt-4 grid grid-cols-3 gap-4">
                {onizlemeler.map((url, index) => (
                  <div key={index} className="relative">
                    ...
                    <button
                      type="button"
                      onClick={() => handleFotografKaldir(index)}
